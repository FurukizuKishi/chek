 ...
 *      The entire tree can be displayed using the display() command, allowing for multiple levels of specificity in
 * regards to the detail shown about each element. Objects can easily be stored, altered and queried using the key path
 * system, allowing the user to specify an exact route to the object they are looking for.
 * @author ck18334
 * @version 1.0
 * @since 2019-11-22
 */
...
public class ElementMap extends HashMap {
    ...
    public Object get(String key, String ... keyPath) {
        ...
        return getBranch(keyPath).get(key);
    }
    ...
    public JPanel parentPanel(Object component) {
        if (component instanceof Component) {
            ...
            for (JPanel panel : mapSet) {
                if (Arrays.asList(panel.getComponents()).contains(component)) {
                    return panel;
                }
            }
            return null;
    ...
    public void expandTree(Map<String, Object> node, int depth, int attributeLevel) {
        ...
        for (Entry<String, Object> childNode : nodeList) {
            String element = indent + "+ " + childNode.getKey();
            if (childNode.getValue() instanceof Map) {
                ...
                if (attributeLevel > 0) {
                    ...
                    if (childNode.getValue() instanceof JButton) {
                        ...
                        if (childNode.getValue() instanceof JTextField) {
